...
module.exports = {
    ...
    .addStringOption(option => option.setName('username').setDescription('The username of the user to unblock').setRequired(true))
    .addStringOption(option => option.setName('wiki').setDescription('The wiki on which to unblock the user').setRequired(false))
    .addStringOption(option => option.setName('lang').setDescription('wiki language code').setRequired(false))
    .addStringOption(option => option.setName('reason').setDescription('The reason for unblocking the user').setRequired(false)),
    async execute(interaction) {
        ...
        const blockRes = await client({
            ...
            data: new URLSearchParams({
                ...
                reason: `[DISCORD] Unblocked by ${interaction.user.tag}, ${interaction.user.id}: ${reason ? reason : 'No reason provided'}`,
                bot: true
            })
        });
        const blockResult = JSON.parse(blockRes.data);
        if (blockResult.error) {
            interaction.editReply({ content: `Failed to unblock user. Error: ${blockResult.error.info}` });
            return;
        }
        interaction.editReply({ content: `Successfully unblocked user ${username}.` });
    }
};
