...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('move') 
        .setDescription('move a page')
        .addStringOption(option => option.setName('title').setDescription('The title of the page to move').setRequired(true))
        .addStringOption(option => option.setName('newtitle').setDescription('The new title of the page').setRequired(true))
        .addStringOption(option => option.setName('wiki').setDescription('The wiki to move the page on').setRequired(false))
        .addStringOption(option => option.setName('lang').setDescription('wiki language code').setRequired(false))
        .addStringOption(option => option.setName('reason').setDescription('The reason for moving the page').setRequired(false)),
    ...
    async execute(interaction) {
        ...
        let obj = {
            ...
            to: `${newtitle}`,
            token: `${csrfToken}`,
            reason: `[DISCORD] Moved ${title} to ${newtitle} by ${interaction.user.tag}, ${interaction.user.id}: ${reason ? reason : 'No reason provided'}`,
            bot: true
        };
        ...
        if (appendResponse.error) {
            return interaction.editReply({
                content: `Move failed: ${appendResponse.error.code}\n${appendResponse.error.info}`
            });
        }
        if (appendResponse && appendResponse.move) {
            interaction.editReply({ content: `Page ${title} moved successfully to ${newtitle}.` });
        }
    }   
};
