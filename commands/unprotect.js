...
module.exports = {
    ...
    .setName('unprotect') 
    .setDescription('unprotect a page')
    .addStringOption(option => option.setName('title').setDescription('The title of the page to unprotect').setRequired(true))
    .addStringOption(option => option.setName('wiki').setDescription('The wiki to unprotect the page on').setRequired(false))
    .addStringOption(option => option.setName('lang').setDescription('wiki language code').setRequired(false))
    .addStringOption(option => option.setName('reason').setDescription('The reason for unprotecting the page').setRequired(false)),
    async execute(interaction) {
        ...
        const protectPageRes = await client({
            ...
            data: new URLSearchParams({
                ...
                reason: `[DISCORD] Unprotected by ${interaction.user.tag}, ${interaction.user.id}: ${reason ? reason : 'No reason provided'}`,
                bot: true
            })
        });
        const protectResult = JSON.parse(protectPageRes.data);
        if (protectResult.error) {
            interaction.editReply({ content: `Failed to unprotect page. Error: ${protectResult.error.info}` });
            return;
        }
        interaction.editReply({ content: `Successfully unprotected page ${title}.` });
    }
};