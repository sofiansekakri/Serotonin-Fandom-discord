...
module.exports = {
    ...
    .addStringOption(option => option.setName('username').setDescription('The username of the user to block').setRequired(true))
    .addStringOption(option => option.setName('wiki').setDescription('The wiki on which to block the user').setRequired(false))
    .addStringOption(option => option.setName('lang').setDescription('wiki language code').setRequired(false))
    .addStringOption(option => option.setName('reason').setDescription('The reason for blocking the user').setRequired(false))
    .addStringOption(option => option.setName('duration').setDescription('The duration of the block (e.g. "1 day", "indefinite")').setRequired(false)),
    async execute(interaction) {
        const whitelist = JSON.parse(fs.readFileSync('./sensitive/whitelist.json', 'utf8'));
        if (!whitelist.main.includes(interaction.user.id) && !whitelist.cmod.includes(interaction.user.id) && !whitelist.localConfig[String(interaction.guildId)]?.whitelist?.main.includes(interaction.user.id)) { await interaction.reply({content:'you\'re not permitted to use this command.', flags: 64}); return;}
        ...
        const blockRes = await client({
            ...
            data: new URLSearchParams({
                user: `${username}`,
                action: 'block',
                expiry: `${duration ? duration : 'indefinite'}`,
                format: 'json',
                token: `${csrfTokenFirst}`,
                reason: `[DISCORD] Blocked by ${interaction.user.tag}, ${interaction.user.id}: ${reason ? reason : 'No reason provided'}`,
                bot: true
            })
        });
        const blockResult = JSON.parse(blockRes.data);
        if (blockResult.error) {
            interaction.editReply({ content: `Failed to block user. Error: ${blockResult.error.info}` });
            return;
        }
        interaction.editReply({ content: `Successfully blocked user ${username}.` });
    }
};